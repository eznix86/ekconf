...
func TestEncrypt_DifferentPasswords(t *testing.T) {
	data := []byte("same data")
	ef1, err := Encrypt(data, []byte("pass1"))
	require.NoError(t, err)

	ef2, err := Encrypt(data, []byte("pass2"))
	require.NoError(t, err)

	_, err = Decrypt(ef1, []byte("pass2"))
	require.Error(t, err)

	_, err = Decrypt(ef2, []byte("pass1"))
	require.Error(t, err)

	d1, err := Decrypt(ef1, []byte("pass1"))
	require.NoError(t, err)
	assert.Equal(t, data, d1)

	d2, err := Decrypt(ef2, []byte("pass2"))
	require.NoError(t, err)
...
func TestDeriveKey_DifferentSalt(t *testing.T) {
	k1 := deriveKey([]byte("password"), []byte("0123456789abcdef"))
	k2 := deriveKey([]byte("password"), []byte("fedcba9876543210"))
